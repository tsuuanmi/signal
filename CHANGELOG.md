...
- Development-only `signal.validation_phase_error_characterization/v1` now combines the prepared phase-interpretation dataset with exact biological missing/extra window links to retain complete candidate-derived non-zero/structured quantities, threshold-free window envelopes, signed adjacent-window trajectories, and descriptive error-overlap strata. The exact evaluator-v2 source keeps cases without reviewer proxy coverage explicitly `not_evaluated`; multi-locus event/window links are deduplicated, no-error-overlap windows are never treated as clean truth, and the artifact introduces no candidate selection, phase state, threshold, recovery rule, weighting, holdout feature exposure, or production behavior.
