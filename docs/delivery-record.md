...
- compact analysis v6 JSON exposes input/reference/configuration hashes, call count/trim, merged noisy regions, alignment summary, normalized variants with reviewer-facing reference-oriented peak/quality evidence, and warning counts; effective parameters remain in strict configuration schema v5;
- internal coordinates and external 1-based variants are explicit;
- circular rCRS origin-spanning reads are representable;
- analysis v6 omits filenames, full sequences/windows/gapped rows, method constants, full peaks, and vendor data; basecalls v1 includes full sequences but omits reference/alignment/variant evidence; sample evidence v4 factors read identity/orientation/coverage once, emits concise overlap/admission evidence plus differential loci and normalized variant support, and omits consensus/interpretation; no current contract emits compatibility output, genotype, heteroplasmy fraction, clinical meaning, VCF/BCF, or hidden regional correction;
