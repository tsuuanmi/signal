...
## 23. Pairwise consensus and multi-trace assembly are different abstractions

Tracy exposes two separate commands that are easy to conflate.

The pairwise consensus command requires exactly two traces. Each trace is
basecalled, trimmed, and converted to a profile independently. The second trace
is tested in both orientations relative to the first, an overlap alignment is
created, and the command requires both a minimum overlap and minimum match
fraction before combining profile evidence.

This is a useful implementation for one F/R pair, but it is intentionally a
two-read abstraction.

The assembly command accepts an arbitrary trace collection. In reference-guided
mode every trace is independently basecalled, trimmed, profiled, evaluated in
both orientations against the reference, and either admitted or rejected before
multi-read alignment begins.

### Signal lesson

The reusable architectural rule is not "merge forward and reverse". It is:

~~~text
process each trace independently
then reconcile admitted read observations
~~~

For a tiled mtDNA sample, canonical F/R pairing is only one relationship among
reads.

## 24. Cross-amplicon overlap makes pair-first merging lossy

A realistic sample can contain:
...
where HV2F and HV3R overlap the same reference coordinates despite belonging to
different amplicons.

If the pipeline first collapses every named F/R pair into an amplicon consensus,
the original read topology is partly lost before cross-amplicon evidence is
evaluated.

### Signal lesson

All admitted reads should meet in reference-coordinate/event space. Direction,
amplicon, primer, and replicate identifiers remain support/provenance dimensions
rather than exclusive merge keys.

A missing canonical partner such as HV2R must not make HV2F unusable when other
reads cover the same coordinates.

## 25. Tracy progressively aligns profiles but finally collapses evidence

Reference-guided Tracy assembly sorts accepted traces by their reference
alignment score, aligns the best trace to the reference, and then progressively
aligns each next trace profile against the growing alignment profile.

This is useful evidence that progressive profile alignment can reconcile
arbitrary overlapping reads and orientations.

However, the final assembly consensus is still generated from alignment
characters by column counting. Thus the profile evidence used to build the
layout is not preserved through the final scientific decision.

### Signal lesson

Signal should stop one layer earlier:
...
A consensus string can be projected afterwards, but sample variants and
discordance states should derive directly from aggregated evidence.

## 26. F/R support and cross-amplicon support are independent dimensions

A single ordinal support label cannot faithfully distinguish:
...
These patterns have different artifact structure.

### Signal lesson

Record support topology as factorized dimensions such as read count, direction
counts, amplicon count, and replicate grouping. Do not call different amplicons
"independent" unless the assay design actually supports that interpretation.

## 27. Consensus sequence should be a view, not the sample truth model

Tracy's final user-facing artifact is naturally a consensus sequence. For Signal
that should be an interoperability/view layer.

The authoritative sample model needs to represent states that a sequence cannot:

- high-quality read disagreement;
- reproducible mixed evidence;
- competing indel placements;
- local no-coverage;
- single-direction coverage;
- cross-amplicon support topology;
- exact read/call provenance.

Therefore future sample variants should not be generated by simply diffing a
flattened consensus sequence against rCRS.


## 28. Exact mechanism behind Tracy's self-locating reads

For a <=50 kb single FASTA, Tracy does not infer an amplicon class. It solves a
semi-global alignment problem.

The trimmed trace profile is scored against both the forward reference profile
and the reverse-complement reference profile. Tracy selects one orientation and
aligns the trace profile with `AlignConfig<true, false>`.

Because unused reference flanks are free, the trace can align to any internal
reference interval. Tracy then derives the local reference slice from the first
and last alignment columns containing trace bases.

Thus the covered region is a consequence of traceback, not input metadata.

Reference-guided `assemble` similarly scores every independently basecalled and
trimmed read in forward and reverse-complement orientation against the complete
reference before accepting it into the progressive assembly.

Pairwise `consensus` is different: its `AlignConfig<true, true>` is an
end-free overlap alignment between two traces and therefore discovers only
relative overlap, not genomic coordinates.

### Important implementation artifacts not to copy

Tracy's orientation equality semantics differ by command:

~~~text
sage short-reference align:  forward if F > R, else reverse
assemble with reference:     forward if F >= R, else reverse
pairwise consensus:          forward if F > R, else reverse
~~~

So exact ties can produce different orientation labels depending on the command.

Signal's current behavior—explicit failure when forward and reverse remain
equally supported—is a stronger invariant.

## 29. Current Signal already has a cleaner placement implementation

Signal's `alignment::gotoh::align()` implements free reference flanks by
allowing zero-cost starts across the reference and evaluating all reference
endpoints after the complete query is consumed.

The traceback explicitly returns:
...
and `align_best()` performs forward/reverse query evaluation, ambiguity checks,
quality thresholds, and circular projection.

For mtDNA sample analysis, the correct implementation is therefore to preserve
and expose this existing placement result as `ReadObservation`, not to invent a
new HV-region classifier or port Tracy's FM-index path.
