...
- **SRS-SAMPLE-005:** Sample evidence MUST separate covered-locus alignment observations from normalized canonical event observations. Every normalized event support MUST retain the contributing read identity, derived orientation, configured eligibility, and exact exclusion reasons; read-level filtering MUST NOT erase the observation. Absence of a read observation at a locus MUST NOT count as reference support. Future sample variants and discordance states MUST derive from this evidence; a consensus sequence MAY be emitted only as a downstream projection.
