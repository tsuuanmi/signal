...
- **SRS-CFG-007:** Configuration MUST require a positive `sample_reconciliation.minimum_overlap_bases` and finite `sample_reconciliation.minimum_overlap_agreement` in `(0,1]`; these settings apply only after independently placed reads enter sample reconciliation.
...
- **SRS-ALN-006:** An exact best profile-score orientation tie, modulo-distinct placement tie, insufficient callable columns, low identity, over-limit matrix, or invalid traceback MUST fail explicitly. Primary-sequence exact-match/mismatch counts MUST NOT break a profile-score orientation tie.
...
- **SRS-SAMPLE-009:** A materialized overlap pair MUST be eligible for later consensus reconciliation exactly when its comparable canonical-base count is at least `minimum_overlap_bases` and its canonical-base agreement fraction is at least `minimum_overlap_agreement`. Failed rules MUST remain explicit as `overlap_below_minimum` and/or `agreement_below_minimum` without mutating either read.
