...
Poly-C phase, candidate-hypothesis, characterization, parameter-sensitivity, explainability, recurrent-locus context, and opposite-orientation control research generated under `validation-results/research/polyc-*` retains sample IDs, trace hashes, acquisition/PCR metadata, read-local interrupt evidence, reference-oriented nucleotide profiles, tract context, downstream candidate-offset evidence, and derived persistence/trajectory/sensitivity/explainability/recurrent-locus/control summaries. These are sensitive derived biological artifacts and MUST remain ignored local validation data under the same approval, retention, and redistribution policy.
