...
- Draft 2020-12 analysis-v7, basecalls-v2, and sample-evidence-v8 schema validation owned by `scripts/validate_result_schemas.py` and CI, not Rust integration code;
...
- exact circular-rCRS HV1/HV2 phase applicability, explicit tract/window insufficiency, profile-gap window membership, reverse profile projection, circular post-tract distance, complete ±1..±5 candidate retention, and zero/shifted/residual mass decomposition;
...
Production profile geometry remains threshold-free. Empirical threshold research uses the dedicated `signal-validation` binary so clean reference-matching loci are measured as well as differential loci without changing `signal.sample_evidence/v8`.
...
Biological missing/extra disagreements can then be joined to exact development phase windows through `signal.validation_variant_phase_context/v1`, while representation-only disagreements remain outside biological error attribution. `signal.validation_phase_error_characterization/v1` consumes that artifact together with the prepared development dataset and the exact evaluator-v2 source referenced by the context artifact. It retains candidate-level non-zero/structured quantities, threshold-free window envelopes, signed adjacent-window trajectories, and descriptive error-overlap strata. Evaluated windows without error overlap are `none`, while cases absent from evaluator coverage remain `not_evaluated`; neither is a clean/true-negative label. Neither artifact selects a candidate, phase state, threshold, persistence/recovery rule, or production weighting policy.
