...
Biological missing/extra disagreements can then be joined to exact development phase windows through `signal.validation_variant_phase_context/v1`, while representation-only disagreements remain outside biological error attribution. `signal.validation_phase_error_characterization/v1` consumes that artifact together with the prepared development dataset to retain candidate-level non-zero/structured quantities, threshold-free window envelopes, signed adjacent-window trajectories, and descriptive error-overlap strata. The `none` overlap context is not a clean/true-negative label, and neither artifact selects a candidate, phase state, threshold, persistence/recovery rule, or production weighting policy.
