...
All core commands (`analyze`, `basecall`, and `sample`) load exactly one strict TOML file selected by `SIGNAL_CONFIG` or `config/signal.toml`. The binary does not parse `.env`, and environment variables do not override individual scientific values.
