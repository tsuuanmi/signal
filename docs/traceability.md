...
| `SRS-ALN-*` | `pipeline.md` Stage 5, ADR-0029 + ADR-0047 + read-observation boundary | `src/alignment/`, `src/model/locus_evidence.rs`, `src/model/alignment.rs`, `src/model/read_observation.rs` | fixed-point profile scoring, one-hot compatibility, unresolved-profile, orientation/traceback/circular tests; canonical right-gap fixtures required by ADR-0047 | analysis alignment summary |
