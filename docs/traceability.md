...
| `SRS-SIG-*` | `signal-processing.md`, ADR-0028 | `src/locus.rs`, `src/model/locus_evidence.rs`, `src/signal_processing/` | locus geometry/profile tests + signal feature boundary tests | internal evidence profile + noisy-region projections |
...
| `SRS-SAMPLE-*` | ADR-0023, ADR-0030, architecture invariants | `src/sample/`, `src/model/sample_evidence.rs`, `src/pipeline/sample.rs` | deterministic overlap/admission unit tests, sparse sample evidence tests, end-to-end CLI/schema validation, local non-committed multi-read review | `signal.sample_evidence/v4` |
