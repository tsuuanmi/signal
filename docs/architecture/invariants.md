...
- **INV-SAMPLE-015:** Nucleotide contribution eligibility is structural evidence availability, not quality adjudication: call-backed observations with a real `EvidenceProfile` are eligible, missing-profile calls are not, and deletions are separate events. Unresolved calls and candidate-noisy membership cannot by themselves suppress real profile evidence.
