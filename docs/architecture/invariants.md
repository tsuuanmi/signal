...
- **INV-SAMPLE-003:** Sample evidence defines read identity, reviewer-facing name, orientation, and coverage once in a SHA-sorted read registry. Locus and normalized-variant evidence reference that registry by deterministic read index and MUST NOT duplicate read identity/orientation fields.
