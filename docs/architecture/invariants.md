...
- **INV-SAMPLE-003:** Normalized variant support is keyed by biological variant identity and preserves each contributing read name, SHA-256, derived orientation, configured eligibility, exclusion reasons, and concise original-call mappings; read-level filtering MUST NOT erase a normalized observation.
