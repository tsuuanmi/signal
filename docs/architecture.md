...
| `report` | analysis-v5/basecalls-v1/sample-evidence-v2 projection, shared serialization, atomic publish | scientific decisions and compatibility output |
...
Compact `signal.analysis/v5` projects one completed read observation. `signal.basecalls/v1` projects the shared reference-independent stages. `signal.sample_evidence/v2` projects independently placed reads into one deterministic read registry, sparse differential-locus observations, and normalized variant support. Read identity/orientation/coverage are factored once at top level; locus and variant records reference reads by index. It omits dense all-reference loci and contains no consensus or sample-level variant verdict. Configuration remains schema version 4, and no compatibility result is assembled.
