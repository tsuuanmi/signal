...
`docs/src/` mirrors first-party Rust source ownership. Every `src/**/*.rs` file that is covered by the repository mirror policy has a same-relative-path manual here.
...
8. **Traceability** — relevant SRS requirements, ADRs, public contracts, and tests.
...
The mirror is descriptive implementation documentation. It does not override the SRS, contracts, accepted ADRs, or method documentation.
