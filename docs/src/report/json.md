...
Assembles the compact `signal.analysis/v4` document and serializes it
deterministically.

## Responsibilities

- Build the immutable `AnalysisResult` from the completed stage outputs,
  including rolling signal windows and merged noisy regions.
...
- `build(completed) -> Result<AnalysisResult>`: assembles the v4 document.
...
- `schema_version` is `signal.analysis/v4`; signal processing is
  `signal.windowed_snr/v1`; variant calling is `signal.primary_difference/v3`.
...
- `model::result`, `model::alignment`, `model::basecalls`, `model::quality`,
  `model::reference`, `model::signal`, `model::trace`, `model::variant`.
...
The document records rolling signal annotations, the selected sequence,
quality/trim bounds, the winning alignment, and reported variants with only their local peak/quality evidence, plus
...
Integration tests exercise deterministic report assembly and key output fields.
`scripts/validate_analysis_schema.py` and CI validate the schema, bundled example,
optional generated results, and negative signal/call-mapping shapes.
