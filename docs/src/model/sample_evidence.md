...
- Keep retained locus observations as deterministic read index, optional original call index, state/base/quality, one optional reference-oriented `CallSignalEvidence` containing corrected A/C/G/T amplitudes, per-channel SNR, optional basecall-independent `EvidenceProfile`, existing merged noisy-region membership, and one explicit structural nucleotide-contribution state. Deletions have no source call index.
