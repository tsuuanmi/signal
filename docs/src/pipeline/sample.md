...
- Write exactly one persistent `$SIGNAL_LOG_DIR/<sample-id>.sample.log`; all nested per-trace read/basecall/signal/QC/alignment/variant records use that same logger between `sample_read_started` and `sample_read_completed`, with no per-trace log opened by the sample command.
